    ...
    public class AgentUpdateController : ControllerBase
    {
        ...
        public async Task<ActionResult> DownloadPackage(string platform, string downloadId)
        {
            try
            {
                ...
                {
                    case "win-x64":
                        filePath = Path.Combine(HostEnv.WebRootPath, "Content", "Tess-Win10-x64.zip");
                        break;
                    case "win-x86":
                        filePath = Path.Combine(HostEnv.WebRootPath, "Content", "Tess-Win10-x86.zip");
                        break;
                    case "linux":
                        filePath = Path.Combine(HostEnv.WebRootPath, "Content", "Tess-Linux.zip");
                        break;
                    case "macos-x64":
                        filePath = Path.Combine(HostEnv.WebRootPath, "Content", "Tess-MacOS-x64.zip");
                        break;
                    default:
                ...
                return File(fileStream, "application/octet-stream", "TessUpdate.zip");
            }
            catch (Exception ex)
            {
                _downloadingAgents.Remove(downloadId);
        ...
        private async Task<bool> CheckForDeviceBan(string deviceIp)
        {
            ...
            if (AppConfig.BannedDevices.Contains(deviceIp))
            {
                ...
                foreach (var bannedDevice in bannedDevices)
                {
                    ...
                    var command = "sc delete Tess_Service & taskkill /im Tess_Agent.exe /f";
