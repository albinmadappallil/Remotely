...
using Tess.Server.Components.ModalContents;
...
namespace Tess.Server.Pages
{
    public partial class ManageOrganization : AuthComponentBase
    {
        ...
        private readonly List<TessUser> _orgUsers = new();
        ...
        [Inject]
        private UserManager<TessUser> UserManager { get; set; }
        ...
        private async Task DeleteUser(TessUser user)
        {
            if (!User.IsAdministrator)
            {
                return;
        ...
        private async Task EditDeviceGroups(TessUser user)
        {
        ...
        private async Task ResetPassword(TessUser user)
        {
            if (!User.IsAdministrator)
            {
                return;
        ...
        private async Task SendInvite()
        {
            ...
            else
            {
                ...
                var emailResult = await EmailSender.SendEmailAsync(invite.InvitedUser, "Invitation to Organization in Tess",
                        $@"<img src='{NavManager.BaseUri}images/Tess_Logo.png'/>
                            <br><br>
                            Hello!
                            <br><br>
        ...
        private void SetUserIsAdmin(ChangeEventArgs args, TessUser orgUser)
        {
            if (!User.IsAdministrator)
            {
                return;
