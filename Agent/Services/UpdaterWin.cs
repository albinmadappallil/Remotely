    ...
    public class UpdaterWin : IUpdater
    {
        ...
        public async Task CheckForUpdates()
        {
            try
            {
                ...
                var fileUrl = serverUrl + $"/Content/Tess-Win10-{platform}.zip";
        ...
        public async Task InstallLatestVersion()
        {
            try
            {
                ...
                var installerPath = Path.Combine(Path.GetTempPath(), "Tess_Installer.exe");
                var platform = Environment.Is64BitOperatingSystem ? "x64" : "x86";

                await _webClientEx.DownloadFileTaskAsync(
                     serverUrl + $"/Content/Tess_Installer.exe",
                     installerPath);
                ...
                foreach (var proc in Process.GetProcessesByName("Tess_Installer"))
                {
                    proc.Kill();
                }
